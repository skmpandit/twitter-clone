...
function TweetBox({setTweets}: Props) {
  ...
  return (
    ...
    <img className='h-14 w-14 mt-4 object-cover rounded-full' src={session?.user?.image || "https://links.papareact.com/gll"} alt="" />
        <div className='flex flex-1 items-center pl-2'>
            <form className='flex flex-1 flex-col'>
                ...
                <div className='flex items-center'>
                    ...
                    <button onClick={handleSubmit} disabled={!input || !session} className='py-2 px-5 font-bold rounded-full bg-twitter text-white disabled:opacity-40'>Tweet</button>
                </div>
                {imageUrlBoxIsOpen && (
                ...
                {image && (
                    <img className='mt-10 h-40 w-full rounded-xl object-cover shadow-lg' src={image} alt="" />
                )}
            </form>
        </div>
    </div>
